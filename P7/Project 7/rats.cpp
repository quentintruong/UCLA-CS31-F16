...
void Rat::move()
{
    ...
    if (attemptMove(*m_arena, dir, m_row, m_col) == true){
...
int Arena::numberOfRatsAt(int r, int c) const
{
    ...
    int grid[m_rows + 1][m_cols + 1];
    for (int myR = 1; myR <= m_rows; myR++)
        for (int myC = 1; myC <= m_cols; myC++)
            grid[myR][myC] = 0;
    
    for (int a = 0; a < m_nRats; a++){
        grid[(m_rats[a])->row()][(m_rats[a])->col()] = grid[(m_rats[a])->row()][(m_rats[a])->col()] + 1;
    }
    return grid[r][c];
}
...
void Arena::display(string msg) const
{
    ...
    for (int a = 0; a < m_nRats; a++){
        if (displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] == '.' ||
            displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] == '*')
            displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] = 'R';
        else if (displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] == 'R')
            displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] = '2';
        else if (displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] >= '2' &&
                 displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] < '8')
            displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] = displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] + 1;
        else
            displayGrid[(m_rats[a])->row() - 1][(m_rats[a])->col() - 1] = '9';
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    int nRatN = a.numberOfRatsAt(r-2, c) +
                a.numberOfRatsAt(r-1, c+1) +
                a.numberOfRatsAt(r-1, c-1);
    int nRatE = a.numberOfRatsAt(r+1, c+1) +
                a.numberOfRatsAt(r, c+2) +
                a.numberOfRatsAt(r-1, c+1);
    int nRatS = a.numberOfRatsAt(r+1, c+1) +
                a.numberOfRatsAt(r+2, c) +
                a.numberOfRatsAt(r+1, c-1);
    int nRatW = a.numberOfRatsAt(r+1, c-1) +
                a.numberOfRatsAt(r-1, c-1) +
                a.numberOfRatsAt(r, c-2);
    int nRatC = a.numberOfRatsAt(r-1, c) +
                a.numberOfRatsAt(r, c+1) +
                a.numberOfRatsAt(r+1, c) +
                a.numberOfRatsAt(r, c-1);
    
    if (a.numberOfRatsAt(r-1, c) > 0)//dont travel to pos w a rat
        nRatN = 9999;
    if (a.numberOfRatsAt(r, c+1) > 0)
        nRatE = 9999;
    if (a.numberOfRatsAt(r+1, c) > 0)
        nRatS = 9999;
    if (a.numberOfRatsAt(r, c-1) > 0)
        nRatW = 9999;
    
    if (a.player()->row() == 1)//dont travel to an edge
        nRatN = 9999;
    if (a.player()->col() == a.cols())
        nRatE = 9999;
    if (a.player()->row() == a.rows())
        nRatS = 9999;
    if (a.player()->col() == 1)
        nRatW = 9999;
    
    int myNRats[NUMDIRS] = {nRatN, nRatE, nRatS, nRatW};
    
    int minP = 0;
    int otherP = 1;
    while (minP != NUMDIRS && otherP != NUMDIRS){
        if (myNRats[minP] < myNRats[otherP])
            otherP++;
        else{
            minP = otherP;
            otherP++;
    ...
    //if surrounded by rats, drop a poison
    //otherwise, move
    //if north has the least, go north (same for other dirs)
    //if all are empty and there is no poison, drop a poison
    //if all are empty and there is a poison, go randomly
    
    //cout << "N: " << nRatN << ", E: " << nRatE << ", S: " << nRatS << ", W: " << nRatW << ", C: " << nRatC << ", M: " << minP;
    if (nRatN > 0 && nRatE > 0 && nRatS > 0 && nRatW > 0)//surrounded, must stay put
        return false;
    else if (nRatC == 0 && a.getCellStatus(r, c) == EMPTY)//has nothing around it, must put a poison
        return false;
    else
        bestDir = minP;
    return true;
